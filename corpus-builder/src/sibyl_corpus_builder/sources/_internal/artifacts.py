"""Raw/canonical source artifact cache.

Pipeline position:

    source adapter download/import -> THIS MODULE -> raw.bin + canonical.txt + artifact.json
                                               -> source preparation

The cache records both raw and canonical SHA-256 values plus the versioned normalizer identity.
This allows later preparation to reproduce/pin the exact text without silently redownloading or
renormalizing it.
"""
...
from sibyl_corpus_core.hashing import sha256_bytes, sha256_text

from .adapters import canonicalize_source
from .registry import RegistryTextVersion, RegistryWork
...
def artifact_directory(cache_dir: Path, work_id: str, text_version_id: str) -> Path:
    """Returns the deterministic cache directory for one concrete text version."""
...
def write_source_artifact(
    *,
    ...
    """Normalizes and atomically caches one concrete raw source representation."""
    canonical_text, normalizer = canonicalize_source(
        raw,
...
def read_source_artifact(cache_dir: Path, work_id: str, text_version_id: str) -> SourceArtifact:
    """Loads a cached source artifact and verifies raw/canonical checksums before reuse."""
    ...
    return SourceArtifact(
        ...
        normalizer=normalizer,
        artifact_kind=str(
            metadata.get("artifact_kind") or _artifact_kind_from_normalizer(normalizer)
        ),
