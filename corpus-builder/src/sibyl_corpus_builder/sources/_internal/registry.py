"""Typed access to permanent ``corpus-sources`` registry records.

The source-ingestion feature uses this module before acquisition/preparation to resolve one
concrete text version and enforce approval/rights policy. It does not own registry validation
for the repository as a whole; that remains in ``corpus-sources``.
"""
...
class RegistryWork:
    ...
    def text_version(self, version_id: str | None = None) -> RegistryTextVersion:
        """Resolves a text version, requiring an ID when multiple versions exist."""
